...
uint16_t wyoki_X,wyoki_Y ;
// Gert : parse wyoki device choice
void Wyoki_device_setup(char *d)
{
	strcpy(device,d);
	L("device arg : <%s>\n",device);
	if (strcmp(device,"protoype")!=0) {
		wyoki_X = 320 ;
		wyoki_Y = 240 ;
	L("Selected prototype wyoki device\n");
	sendMsgToGui("~SHOW|device prototype selected\n");
	} else {
		wyoki_X = 100 ;
		wyoki_Y = 100 ;
	L("no device selected, huh ?\n");
	sendMsgToGui("~SHOW|booboo, no device selected\n");
	}
}
...
{  
  ...
  // Gert fixed screensize based on wyoki device
  screenformat.width = wyoki_X ;
  screenformat.height = wyoki_Y ;
