...
// Gert wyoki device types
enum wyoki_type {PROTOTYPE,WATCH,BROCHE,EYEPATCH};
enum wyoki_type wyoki=PROTOTYPE;
...
// Gert
// Scale image to fit on display ... enum wyoki_type
// How does this work ?? It doesn't full scale + xy cut is best option
rfbNewClientHookPtr clientHook(rfbClientPtr cl)
{
  if (scaling!=100)
  {
    rfbScalingSetup(cl, vncscr->width*scaling/100.0, vncscr->height*scaling/100.0);
    // Test prototype device hardcode
    //rfbScalingSetup(cl, 320, 240);
...
{ 
  ...
  // Gert TODO : find better samplesPerPixel ratio, for smoother framegrabbing ... read docs :(
  if (rotation==0 || rotation==180) 
  vncscr = rfbGetScreen(&argc, argv, screenformat.width , screenformat.height, 0 /* not used */ , 1,  screenformat.bitsPerPixel/CHAR_BIT);
  else
  vncscr = rfbGetScreen(&argc, argv, screenformat.height, screenformat.width, 0 /* not used */ , 1,  screenformat.bitsPerPixel/CHAR_BIT);

  assert(vncscr != NULL);

  vncscr->desktopName = "Wyoki daemon";
  ...
  rfbInitServer(vncscr);
    ...
    // Gert : set fixed at 8bit depth for speed test
...
void printUsage(char **argv)
{
  L("\nwyokiserver [parameters]\n"
    ...
    "-d <wyoki_device>\t- Wyoki device\n\tprototype : het adafruit breakoutbordje\n\twatch: wyoki watch\n\tbroche: the Wyoki broche\n\teye: the wyoki eyetoy hihi\n"
...
int main(int argc, char **argv)
{
  ...
  if(argc > 1) {
    ...
    while(i < argc) {
	  ...
	  case 'd':
	  i++;
		if(!strcmp(argv[i],"prototype")){
			wyoki = PROTOTYPE ;
			L("Wyoki PROTOTYPE device selected\n");
		} else if (!strcmp(argv[i],"watch")){
			wyoki = WATCH ;
			L("Wyoki WATCH device selected\n");
		} else {
		L("You must be dreaming, there is no \"%s\" available ... yet\n",argv[i]);
		}
	  // Gert grab 100% scaled part of mobile device -> framebuffer.c function called
	  Wyoki_device_setup(argv[i]);
	  break;
          case 'R':
          i++;
