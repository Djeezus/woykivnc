...
public class ServerManager extends Service {
	...
	public static String SOCKET_ADDRESS = "org.godsells.wyoki.gui";
	...
	public void startServer() {
		...
		try {
			...
			String device = preferences.getString("device", "prototype");
			String device_string = "";
			if (!device.equals(""))
				device_string = "-d " + device;				
			...
			String wyokidaemon_exec=getFilesDir().getParent() + "/lib/libwyokiserver.so";
			File f=new File (wyokidaemon_exec);
			if (!f.exists())
			{
				String e="Error! Could not find daemon file, " + wyokidaemon_exec;
				showTextOnScreen(e);
				log(e);
				return;
			}
			...
			Runtime.getRuntime().exec("chmod 777 " + wyokidaemon_exec);
 
			String permission_string="chmod 777 " + wyokidaemon_exec;
			String server_string= wyokidaemon_exec  + " " + device_string + " " + password_check + " " + rotation+ " " + scaling_string + " " + port_string + " "
			...
			log("Starting " + wyokidaemon_exec  + " " + device_string + " " + rotation+ " " + scaling_string + " " + port_string + " "
					+ reverse_string + " " + display_method + " " + display_zte);
			...
			try {
				...
				while (!finished) {
					...
					if (resp.length() > 5
						...
						Intent intent = new Intent("org.godsells.wyoki.ACTIVITY_UPDATE");
	...
	public void showClientConnected(String c) {
		...
		CharSequence tickerText = c + " connected to Wyoki daemon";
		long when = System.currentTimeMillis();

		Notification notification = new Notification(icon, tickerText, when);

		Context context = getApplicationContext();
		CharSequence contentTitle = "Wyoki daemon";
	...
	public void onDestroy() {
		super.onDestroy();
		//showTextOnScreen("Wyoki daemon killed...");
	}
